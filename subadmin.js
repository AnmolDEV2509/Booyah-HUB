...
import { 
    ...
    getDoc, serverTimestamp, query, where, getDocs 
...
/* ==========================================================================
   STRICT SUB-ADMIN ACCESS VERIFICATION
   ========================================================================== */
onAuthStateChanged(auth, async (user) => {
    if (user) {
        // Verify if logged in user exists in Firestore 'sub_admins' collection
        const subAdminDoc = await getDoc(doc(db, "sub_admins", user.email.toLowerCase()));
        
        if (!subAdminDoc.exists()) {
            alert("Access Denied! Aap Sub-Admin nahi hain.");
...
window.createMatch = async () => {
    ...
    try {
        await addDoc(collection(db, "tournaments"), {
            ...
            roomId: '',
            roomPass: '',
            createdBy: auth.currentUser ? auth.currentUser.email : 'SubAdmin',
            createdAt: serverTimestamp()
        });
